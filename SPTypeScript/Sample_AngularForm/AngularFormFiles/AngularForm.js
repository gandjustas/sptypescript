...
(function (_) {
    ...
    function init() {
        CSR.override(105001).onPreRender(function (ctx) {
            ctx.controllerName = controllerName;
            ctx.templateUri = _spPageContextInfo.webServerRelativeUrl + webRelativeTemplatePath;
        }).item('<table width="100%" class="ms-formtable" style="margin-top: 8px;" border="0" cellspacing="0" cellpadding="0" ng-controller=<#=ctx.controllerName#>>' + '<tr ng-repeat="fld in Fields" ng-sp-form-row="fld" >' + '<td width="113" class="ms-formlabel" nowrap="true" valign="top">' + '<h3 class="ms-standardheader" >' + '<nobr>{{fld.Title}}<span ng-if="fld.Required" title="This field is required." class="ms-accentText" >*</span></nobr>' + '</h3>' + '</td>' + '<td width="350" class="ms-formbody" valign="top" ng-sp-field="fld" ></td>' + '</tr>' + '</table>').onPostRender(ngInit).register();
    }
    ...
    function ngInit(ctx) {
        angular.module(moduleName, []).controller(controllerName, ['$scope', 'RenderContext', formController]).directive('ngSpField', ['RenderContext', ngSpField]).directive('ngSpFormRow', ngSpFormRow).value('RenderContext', ctx);
